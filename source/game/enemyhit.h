...
{
public:
	enemyhit(std::string sprite_filename, uint32_t frame_width, uint32_t frame_height, uint32_t num_frames);
	~enemyhit();
	...
	static engine::ref<enemyhit> create(std::string sprite_filename, uint32_t frame_width, uint32_t frame_height, uint32_t num_frames);
private:
