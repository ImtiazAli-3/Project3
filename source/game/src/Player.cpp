...
player::player(): m_timer(0.0f), m_speed(2.5f)
{
	;
}
...
{
	//player class for the character, this class controls the movement and camera techniques
	m_object = object;
	m_object->set_forward(glm::vec3(0.f, 0.f, -1.f));
	m_object->set_position(glm::vec3(0.f, 0.5f, 30.f));
...
void player::on_update(const engine::timestep& time_step)
{
	...
	//only moves if the W key is pressed, A and D is used to turn the character, S is used to walk back
	...
	if (engine::input::key_pressed(engine::key_codes::KEY_D)) // right
		turn(-2.0f * time_step);
	if (engine::input::key_pressed(engine::key_codes::KEY_S)) { // right
		m_object->set_position(m_object->position() -= m_object->forward() * m_speed * (float)time_step);
		m_object->set_rotation_amount(atan2(-m_object->forward().x, -m_object->forward().z));
	}
...
void player::update_camera(engine::perspective_camera& camera)
{
	...
	//camera is placed behind the player but can be moved around the player
	//std::cout << cam_pos;
	//std::cout << cam_look_at;
}
