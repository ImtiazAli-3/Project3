#include "pch.h"
#include "enemyhit.h"
//code for when an enemy is hit by a projectile
enemyhit::enemyhit(std::string sprite_filename, uint32_t frames_x, uint32_t frames_y, uint32_t num_frames)
{
	//takes a file from my assets
	//it will be activated in the example.cpp file
...
void enemyhit::on_update(const engine::timestep& time_step)
{
	if (!s_active)
		return;
...
void enemyhit::on_render(const engine::perspective_camera& camera, engine::ref<engine::shader> shader)
{
	if (!s_active)
		return;
...
void enemyhit::activate(glm::vec3 position, float width, float height)
{
	s_active = true;
	m_timer = 0.0f;
	m_current_frame = 0;
...
engine::ref<enemyhit> enemyhit::create(std::string sprite_filename, uint32_t frame_width, uint32_t frame_height, uint32_t num_frames)
{
	return std::make_shared<enemyhit>(sprite_filename, frame_width, frame_height, num_frames);
}
